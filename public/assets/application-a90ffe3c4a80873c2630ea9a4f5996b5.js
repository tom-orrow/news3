...
(function() {
  var close_alert_on_click, current_page, get_more_articles_on_scroll, hoverTimer, navbarShown, no_more_articles_left, prepare_autosizing, prepare_navbar, prepare_reviews_carousel, resize_blocks, reviews_carousel_offset, root, scroll_update_locked, set_admin_delete_article_ajax, set_admin_delete_comment_ajax, set_article_preview_ajax, set_authentication_ajax, set_index_header_articles_hovering, set_other_actions, show_page_sidebox;
  ...
  $(document).ready(function() {
    ...
    return get_more_articles_on_scroll();
  });

  set_index_header_articles_hovering = function() {
    return $('#index_header_articles a').hover(function() {
      return $(this).parent().toggleClass('highlight');
    });
  };
  ...
  prepare_navbar = function() {
    if ($('#index_header_articles').length > 0) {
      $(window).scroll(function(e) {
        if ($(this).scrollTop() > 450) {
          return $('#header_navbar').addClass('navbar-fixed-top');
        } else {
          return $('#header_navbar').removeClass('navbar-fixed-top');
        }
      });
    } else {
    ...
    $('#categories_menu li').hover(function() {
      ...
      hoveredLi = $(this);
      if (navbarShown) {
        return hoveredLi.children('.nav_sub').show();
      } else {
        return hoverTimer = setTimeout(function() {
          navbarShown = true;
          return hoveredLi.children('.nav_sub').show();
        }, 300);
      }
      ...
      return $(this).find('.nav_sub_categories > li:first-child').addClass('active');
    });
    $('#categories_menu').mouseleave(function() {
      return navbarShown = false;
    });
    $('.nav_sub_categories > li:not(:last-child) a').hover(function() {
      $(this).parent().siblings('li').removeClass('active');
      return $(this).parent().addClass('active');
    });
    return $('#search').click(function() {
      return $(this).find('form input').focus();
    });
  };

  reviews_carousel_offset = 0;

  prepare_reviews_carousel = function() {
    return $('.reviews_carousel_wrapper .controls').click(function() {
      if ($(this).hasClass('prev') && !$(this).hasClass('inactive')) {
        $(this).siblings('.reviews_carousel').animate({
          left: (reviews_carousel_offset + 240) + 'px'
        }, 300);
  ...
  resize_blocks = function() {
    var contentWidth, fullHeight, fullWidth;
    ...
    $('#side_box #offers ul').css({
      height: fullHeight - 92 + 'px'
    });
    return $('#side_box #related_categories').css({
      height: fullHeight + 41 + 'px'
    });
  };

  show_page_sidebox = function() {
    var scrollOptions;
    scrollOptions = {
      scrollInertia: 0
    };
    ...
    $('#related_categories ul li').hover(function() {
      return $(this).addClass('highlight');
    }, function() {
      return $(this).removeClass('highlight');
    });
    return $('#related_categories ul li').click(function() {
      var target;
      if (!$(this).hasClass('active')) {
        ...
        return $('#side_box #offers ul#' + target).mCustomScrollbar("scrollTo", "top");
      }
    });
  };

  set_authentication_ajax = function() {
    $('#sign_in_form ul.nav.nav-tabs li a').click(function() {
      var another_box_id, box_id;
    ...
    $('#sign_in_form').on('show.bs.modal', function() {
      return $('div#wrap, div#footer').addClass('blur');
    });
    $('#sign_in_form').on('hide.bs.modal', function() {
      return $('div#wrap, div#footer').removeClass('blur');
    });
    $('#sign_in_form form').bind('ajax:success', function(e, data, status, xhr) {
      $('#sign_in_form p.alert').html('').addClass('hidden');
      if (data.success) {
        return window.location.replace('/');
      } else {
        $('#sign_in_form p.alert').html(data.errors.join('<br />'));
        return $('#sign_in_form p.alert').removeClass('hidden');
      }
    });
    $('#password_recover_link').click(function() {
      $('#auth_forms').hide();
      return $('#restore_password_form').show();
    });
    return $('#password_recovery_cancel').click(function() {
      $('#restore_password_form').hide();
      return $('#auth_forms').show();
    });
  };

  set_other_actions = function() {
    if ($('#user_menu > a').hasClass('signed')) {
      $('#user_menu > a').click(function() {
        return false;
      });
      return $('#user_menu').hover(function() {
        $(this).children('#user_btn').addClass('active');
        return $(this).children('#user_menu_box').show();
      }, function() {
        $(this).children('#user_btn').removeClass('active');
        return $(this).children('#user_menu_box').hide();
      });
    }
  };

  set_article_preview_ajax = function() {
    $('#new_article a#preview_article_btn').click(function(event) {
      event.preventDefault();
      return $.post($(this).attr('href'), $('#new_article form').serialize(), function(data) {
        var reader, title_pic;
        if (typeof data.errors !== 'undefined') {
          ...
          if (title_pic !== void 0 && title_pic.length > 0) {
            ...
            return reader.readAsDataURL(title_pic);
          }
        }
      });
    });
    return $('#new_article a#edit_article_btn').click(function(event) {
      event.preventDefault();
  ...
  get_more_articles_on_scroll = function() {
    var autoload;
    ...
    if (autoload.length > 0) {
      return $(window).scroll(function() {
        if (document.documentElement.clientHeight + $(document).scrollTop() >= $(document).height() && !scroll_update_locked && !no_more_articles_left) {
          ...
          return $.post(autoload.attr("data-url"), {
            parent_action: autoload.attr("data-parent"),
            page: current_page
          }).done(function(data) {
            return autoload.append(data);
          }).fail(function() {
            return no_more_articles_left = true;
          }).always(function() {
            scroll_update_locked = false;
            current_page = current_page + 1;
            return autoload.removeClass("loading");
          });
        }
      });
    }
  ...
  set_admin_delete_article_ajax = function() {
    return $('div.span5.content-box .button-delete').bind('ajax:success', function(e, data, status, xhr) {
      if (data.success) {
  ...
  set_admin_delete_comment_ajax = function() {
    return $('div.comments .button-delete').bind('ajax:success', function(e, data, status, xhr) {
      if (data.success) {
        return $(this).parent().parent().parent().fadeOut('slow', function() {
          return $(this).remove();
        });
      }
    });
  ...
  root.reply_comment = function(id, elem) {
    var fullname, parent;
    ...
    return $('button#reset_reply').show();
  };

  root.empty_comment_parent_on_click = function() {
    $('button#reset_reply').hide();
    $('#comment_parent_id').val(null);
    $('div.media-body.reply').removeClass('reply');
    return $('#new_comment h2').html('Add Comment');
  };

}).call(this);
(function() {
  var check_default_validations, check_email_field, check_password_confirm_field, check_password_field, check_text_field, check_textarea_field, display_error, error, form_valid, get_field_type, remove_error, validate_field, validate_form;
  ...
  $(document).ready(function() {
    ...
    return $('form.validate input').focusout(function() {
      return validate_field($(this));
    });
  });
  ...
  validate_form = function(form) {
    form_valid = true;
    $.each(form.find('input:not([type=hidden], [type=checkbox])'), function(i, field) {
  ...
  validate_field = function(field) {
    ...
    if (error) {
      if (!$(field).next().hasClass('input_error')) {
        display_error();
      }
      return form_valid = false;
    } else {
      return remove_error(field);
    }
  };
  ...
  get_field_type = function(field) {
    ...
    if (field.attr('type') === 'password' && field.attr('id').match(/.*_confirmation$/)) {
  ...
  display_error = function() {
    ...
    return $('i[data-toggle="tooltip"]').hover(function() {
      return $(this).append('<span class="tooltip">' + $(this).data('content') + '</span>');
    }, function() {
      return $(this).children('.tooltip').remove();
    });
  };

  remove_error = function(field) {
    return $(field).next('i.input_error').remove();
  };

  check_default_validations = function(field) {
    if ($(field).val().length < 3) {
      error = [field, 'Value is too short. Minimum 3 chars long.'];
    }
    if ($(field).val().length > 20) {
      return error = [field, 'Value is too long. Maximum 20 chars long.'];
    }
  };

  check_text_field = function(field) {};

  check_password_field = function(field) {};

  check_password_confirm_field = function(field) {
    var password_field_id;
    password_field_id = $(field).attr('id').match(/^(.*)_confirmation$/)[1];
    if ($(field).val() !== $(field).siblings('#' + password_field_id).val()) {
      return error = [field, 'Password confirmation and password are not equal.'];
    }
  };

  check_textarea_field = function(field) {};

  check_email_field = function(field) {
    if (!$(field).val().match(/.*@.*\..*/)) {
      return error = [field, 'Wrong email format.'];
    }
  };
